...
# TwoTower-BERT frozen setting:
# - BERT encoder is used only to build/cache text embeddings.
# - Training remains TwoTower + in-batch InfoNCE, aligned with run_twotower_tfidf.py.
...
# Kept for interface symmetry with LoRA/full modes; harmless in frozen mode.
...
python "$SCRIPT_DIR/../../run_twotower_bert.py" \
  ...
  --exp_name "twotower_bert_${TUNE_MODE}_use_agent_id_emb" \
  --soft_eval "$SOFT_EVAL"
